...
BASE_URL: str = os.environ.get('REACT_APP_BACKEND_URL', 'https://voice-girl-streaming.preview.emergentagent.com').rstrip('/')
...
class TestPaymentEndpoints:
    ...
    def test_create_checkout_for_paid_tariff(self, auth_token: str) -> None:
        ...
        response = requests.post(f"{BASE_URL}/api/payments/create-checkout",
            json={
                "tariff_id": "hour",
                "origin_url": "https://voice-girl-streaming.preview.emergentagent.com"
            },
            headers={"Authorization": f"Bearer {auth_token}"}
        )
