...
class TestPaymentFlow:
    ...
    def test_01_test_tariff_immediate_activation(self):
        ...
        response = self.session.post(f"{BASE_URL}/api/payments/create-checkout", json={
            "tariff_id": "test",
            "origin_url": "https://care-platform-dev-5.preview.emergentagent.com"
        })
        ...
        assert user.get("test_used") == True, f"Expected test_used=True, got {user.get('test_used')}"
    ...
    def test_04_week_tariff_returns_stripe_url(self):
        ...
        assert "url" in data, f"Expected 'url' in response, got {data}"
        assert "session_id" in data, f"Expected 'session_id' in response, got {data}"
        print(f"✓ Test 4 PASSED: Week tariff returns Stripe URL")
    ...
    def test_05_month_tariff_returns_stripe_url(self):
        ...
        assert "url" in data, f"Expected 'url' in response, got {data}"
        assert "session_id" in data, f"Expected 'session_id' in response, got {data}"
        print(f"✓ Test 5 PASSED: Month tariff returns Stripe URL")
...
class TestTariffEndpoint:
    ...
    def test_11_tariffs_endpoint_returns_all_tariffs(self):
        ...
        # Verify minutes
        assert tariffs["test"]["minutes"] == 3, f"Test minutes should be 3"
        assert tariffs["hour"]["minutes"] == 60, f"Hour minutes should be 60"
        assert tariffs["week"]["minutes"] == 420, f"Week minutes should be 420"
        assert tariffs["month"]["minutes"] == 1800, f"Month minutes should be 1800"
